...
// broadcast to every connected client over SSE. Mirrors MineskiX's
// lib/mapSelectionState.js pattern: this module owns get/mutate/persist/
// broadcast, routes/tally.js just dispatches a single action switch into it.
//
// Deliberately NOT stored here: which group/match tab a given browser is
// currently looking at. That's local UI state per client (see html/index.html)
// so one scorekeeper switching tabs doesn't yank everyone else's view around.
const fs = require('fs');
